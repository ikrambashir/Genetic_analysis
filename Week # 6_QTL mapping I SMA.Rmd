...
```{r}
maize1 <- read.csv("data/population_Z003.csv")
```

```{r}
keeps <- c("PlantHeight","PZA02676.2")
maize1 = maize1[keeps]
maize1 <- maize1[-c(1),] # excluding the first row just to keep phenotype plus marker data only
```
...
image(t(as.matrix(maize1[,1])), col = rainbow(n = 256, end = 2/3, rev = TRUE), axes = FALSE, xlab = "Plant Height", ylab= "Individuals (N = 191)")
image(t(as.matrix(maize1[,2:ncol(maize1)])), col = c("yellow", "blue"), axes = FALSE, xlab = "Markers (PZA02676.2)")
```

```{r}
image(t(as.matrix(sort(maize1[,1]))), col = rainbow(n = 256, end = 2/3, rev = TRUE), axes = FALSE, xlab = "Plant Height",ylab = "Individuals (N = 191)")
image(t(as.matrix(maize1[order(maize1[,1]),2:ncol(maize1)])), col = c("yellow", "blue"), axes = FALSE, xlab = "Markers (PZA02676.2)")
```

```{r}
summary(lm(PlantHeight ~ PZA02676.2, data = maize1))
```

# Single Marker Analysis with `qtl` package
...
```{r}
plotPXG(maize, "PZA02676.2", pheno.col = "PlantHeight")
```

## LOD Score of markers
