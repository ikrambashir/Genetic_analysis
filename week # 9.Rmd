...
Here, we are using the recombinant inbred line (RIL) population Z003, so these methods might not lead to the same results in your population
...
function plot() combined with the argument type = "p" show the test statistic (LOD score) for each marker\
as points
...
```{r}
maize.perm.mr <- scanone(cross = maize.mds, pheno.col = "PlantHeight", method = "mr", n.perm = 1000, verbose = FALSE)
```
...
In order to know the critical value to declare QTL using permutations, we first run the `scanone()` function with the arguments method = "hk" and n.perm = 1000 . Them the function `summary()` shows the LOD threshold for a given level (in our case, alpha = 0.05 ):
...
Notice we can use `maize.im.sig` to extract the chromosomes `( chr )` and positions `( pos )` of our significant\
QTL:
